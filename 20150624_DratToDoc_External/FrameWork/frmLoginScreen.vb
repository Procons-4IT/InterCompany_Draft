...
#Region "Draft to Document"
    Public Sub cmdDraftToOrder()
        ...
        oRec1 = objRemoteCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        Dim pDraft As SAPbobsCOM.Documents
        oRec.DoQuery("Select * from [@Z_DRFT] where U_Active='Y' and isnull(U_UdfName,'') <>'' order by U_DocType,U_Order ")
        For intRow As Integer = 0 To oRec.RecordCount - 1
            WriteErrorlog("Processing Document Type : " & oRec.Fields.Item("Name").Value, strErrorFileName)
            strqry = "Select DocEntry,DocNum from " & oRec.Fields.Item("U_Table").Value & " where WddStatus='-' and DocStatus='O' and isnull(" & oRec.Fields.Item("U_UdfName").Value & " ,'N')='Y' and ObjType=" & oRec.Fields.Item("Code").Value & " order by DocEntry"
            oRec1.DoQuery(strqry)
            For intLoop As Integer = 0 To oRec1.RecordCount - 1
                WriteErrorlog("Processing Draft Number : " & oRec1.Fields.Item("DocEntry").Value, strErrorFileName)
                pDraft = objRemoteCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oDrafts)
                aDocEntry = oRec1.Fields.Item(0).Value
                pDraft.GetByKey(CInt(aDocEntry))
                If pDraft.SaveDraftToDocument() <> 0 Then
                    WriteErrorlog(objRemoteCompany.GetLastErrorDescription, strErrorFileName)
                Else
                    WriteErrorlog("Draft  : " & oRec1.Fields.Item(0).Value & " Converted successfully . ", strErrorFileName)
                End If
                oRec1.MoveNext()
            Next
            oRec.MoveNext()
...
#Region "Read School Files"
    Public Function mainFunction() As Boolean
        ...
        Try
            ...
            For Each fi In aryFi
                ...
                If PushToDIAPI(oFileDt) = True Then
                    MoveToFolder(currentFile)
                Else
                    traceService("File Completed with some errors. Please correct this and try again . File Name:  " & currentFile)
                End If
    ...
    Private Function getIndustry(ByVal aName As String) As Integer
        Dim oTest As SAPbobsCOM.Recordset
        oTest = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        oTest.DoQuery("SELECT T0.[IndCode], T0.[IndName], T0.[IndDesc] FROM OOND T0 where T0.[IndName]='" & aName & "'")
        ...
        End If
    End Function
    Private Function PushToDIAPI(ByVal oDT As DataTable) As Boolean
        Try
                    ...
                    Try
                        ...
                        If oBP.GetByKey(strCardCode) Then
                            If oDT.Rows(index)("Paying").ToString = "F" Then
                                ...
                                oBP.Website = oDT.Rows(index)("Fatherdayphone").ToString.Replace("""", "")
                            ...
                            Else
                                ...
                                oBP.Website = oDT.Rows(index)("Motherdayphone").ToString.Replace("""", "")
                            ...
                            ost = objRemoteCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                            ost.DoQuery("Select * from CRD1 where CardCode='" & strCardCode & "'")
                            If ost.RecordCount > 0 Then
                                For intAddress As Integer = 0 To oBP.Addresses.Count - 1
                                    oBP.Addresses.SetCurrentLine(intAddress)
                                    '  If oBP.Addresses.AddressName2.ToString.Trim.ToUpper = strCustomer.ToString.Replace("""", "").Trim.ToUpper And oBP.Addresses.AddressName <> "" Then
                                    If oBP.Addresses.AddressName.ToString.Trim.ToUpper = "HOME ADDRESS" Then
                                        blnAExist = True
                                        Dim s As String = oBP.Addresses.AddressName
                                        oBP.Addresses.AddressName = s ' "Home Address"
                                        ...
                                        Catch ex As Exception
                                        End Try
                                        Dim oT1 As SAPbobsCOM.Recordset
                                        oT1 = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                                        oT1.DoQuery("Select * from OCRY where Name ='" & oDT.Rows(index)("State Country").ToString.Trim() & "'")
                                        If oT1.RecordCount > 0 Then
                                            oBP.Addresses.Country = oT1.Fields.Item("Code").Value
                                        Else
                                    ...
                                    End If
                                    'MsgBox(oBP.Addresses.AddressName2)
                                Next


                                If Not blnAExist Then
                                    If oBP.Addresses.Count - 1 > 0 Then
                                        Dim inCount As Integer = oBP.Addresses.Count
                                        oBP.Addresses.Add()
                                        '  oBP.Addresses.SetCurrentLine(oBP.Addresses.Count - 1)
                                        oBP.Addresses.SetCurrentLine(inCount)
                                        oBP.Addresses.AddressName = "Home Address- " & inCount + 1
                                        oBP.Addresses.AddressName2 = strCustomer.ToString.Replace("""", "")
                                    ...
                                    Else
                                        Dim inCount As Integer = 0 ' oBP.Addresses.Count
                                        oBP.Addresses.SetCurrentLine(inCount)
                                        oBP.Addresses.AddressName = "Home Address" ' & inCount ' + 1
                                        oBP.Addresses.AddressName2 = strCustomer.ToString.Replace("""", "")
                                        '   oBP.Addresses.AddressName = strCustomer.ToString.Replace("""", "")
                            ...
                            oBP.Industry = getIndustry(oDT.Rows(index)("Special Conditions Custom").ToString.Replace("""", ""))
                            ...
                            oTes.DoQuery("SELECT T1.[EmlGrpCode], T1.[EmlGrpName] FROM [dbo].[OEGP]  T1 where T1.EmlGrpName='" & oDT.Rows(index)("Section") & "'")
                            ...
                            Dim ot2 As SAPbobsCOM.Recordset
                            ot2 = objRemoteCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                            For intContact As Integer = 0 To oBP.ContactEmployees.Count - 1
                                oBP.ContactEmployees.SetCurrentLine(intContact)
                                If oBP.ContactEmployees.Name.ToString.ToUpper.Trim = oDT.Rows(index)("Student Number").ToString.Replace("""", "").ToString.ToUpper.Trim Then 'Person ID Then
                                    blnExist = True
                                    ...
                                    Dim oT1 As SAPbobsCOM.Recordset
                                    oT1 = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                                    oT1.DoQuery("Select * from OCRY where Name ='" & oDT.Rows(index)("Ethnicity").ToString.Replace("""", "") & "'")
                                    If oT1.RecordCount > 0 Then
                                        oBP.ContactEmployees.PlaceOfBirth = oT1.Fields.Item("Code").Value
                                    Else
                                        oBP.ContactEmployees.CityOfBirth = oDT.Rows(index)("Ethnicity").ToString.Replace("""", "")
                                    End If
                                    ...
                                    oTes.DoQuery("SELECT T1.[EmlGrpCode], T1.[EmlGrpName] FROM [dbo].[OEGP]  T1 where T1.EmlGrpName='" & oDT.Rows(index)("Section") & "'")
                                    If oTes.RecordCount > 0 Then
                                        oBP.ContactEmployees.EmailGroupCode = oTes.Fields.Item(0).Value
                                    End If
                                    ...
                                    oBP.ContactEmployees.Remarks2 = strPassword
                                    ...
                                    If oDT.Rows(index)("Paying").ToString = "F" Then
                                        ...
                                        oBP.ContactEmployees.Address = oDT.Rows(index)("Motherdayphone").ToString.Replace("""", "")
                                    ...
                                    Else
                                        ...
                                        oBP.ContactEmployees.Address = oDT.Rows(index)("Fatherdayphone").ToString.Replace("""", "")
                                ...
                                End If
                                If oDT.Rows(index)("Paying").ToString = "F" Then
                                    Dim stFamname As String = oBP.ContactEmployees.Name
                                    If oBP.ContactEmployees.Name.ToString.Replace("""", "").Trim.ToUpper = strFamilyContName.Trim.ToUpper Then
                            ...
                            If Not blnExist Then
                                If oBP.ContactEmployees.Count - 1 > 0 Then
                                    Dim inCount As Integer = oBP.ContactEmployees.Count
                                    oBP.ContactEmployees.Add()
                                    'oBP.ContactEmployees.SetCurrentLine(oBP.ContactEmployees.Count - 1)
                                    '  MsgBox(oBP.ContactEmployees.Count)
                                    oBP.ContactEmployees.SetCurrentLine(inCount)
                                    oBP.ContactEmployees.Name = oDT.Rows(index)("Student Number").ToString.Replace("""", "") 'Person ID
                                    oBP.ContactEmployees.Position = oDT.Rows(index)("Home Room").ToString.Replace("""", "")
                                    oBP.ContactEmployees.FirstName = oDT.Rows(index)("First Name").ToString.Replace("""", "") 'Contact FirstName
                                    oBP.ContactEmployees.MiddleName = oDT.Rows(index)("Middle Name").ToString.Replace("""", "")
                                    ...
                                    oBP.ContactEmployees.Password = oDT.Rows(index)("Grade Level").ToString.Replace("""", "")
    ...
    Private Sub MoveToFolder(ByVal strExFile As String)
        Try
            ...
            If System.IO.File.Exists(strExFile) = True Then
                ...
                End If
                If Directory.Exists(SuceessFolder) = False Then
                    Directory.CreateDirectory(SuceessFolder)
...
#Region "Events"
    ...
    Private Sub frmLoginScreen_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            strPath = strPath & "\Draft_Document_Log_" & strFilename & ".txt"
            ...
            Else
                ...
                WriteErrorlog("Draft to Document Process Started", sPath)
                Me.Hide()
                Me.Hide()
                '   mainFunction()
                cmdDraftToOrder()
                WriteErrorlog("Draft to Document Process completed", sPath)
                Me.Hide()
                Me.Hide()
